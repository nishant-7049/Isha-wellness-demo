...
const DurationAndConsultation = ({activeStatus, setActiveElement, problem, setProblem,setCompletedSections}) => {
      ...
      {isActive && (
        <form className='flex justify-center items-center flex-wrap gap-4 mx-auto w-full' onSubmit={submitHandler}>
          ...
          <div className='flex flex-col sm:w-1/3 w-1/4 grow'>
            ...
            <select className='p-1 bg-white border-2 rounded-md' value={problem?.since} required onChange={(e)=>{
              setProblem((prev)=>({
                ...prev,
              ...
              {sinces.map((since, index)=>(
                <option value={since} key={index}>{since}</option>
              ))}
            </select> 
          </div>
          {/* } */}
          ...
          <div className='flex flex-col sm:w-1/3 w-1/4 grow'>
            ...
            <select className='p-1 bg-white border-2 rounded-md' value={problem?.consult} required onChange={(e)=>{
              setProblem((prev)=>({
                ...prev,
              ...
              {consulted.map((consult, index)=>(
                <option value={consult} key={index}>{consult}</option>
              ))}
            </select> 
          </div>
          
          <div className='flex-shrink-0 w-full'>
            <input className='flex gap-2 py-1 px-2 ml-auto bg-[#00286b] text-white font-semibold' type='submit' value={"Next"}/>
          </div>
        </form>
      )}
    </div>
